...
export const ProductContextProvider: React.FC<IProps> = ({ children }) => {
  ...
  const getProducts = async (filter?: IProductFilter) => {
    ...
    const url = `${process.env.EXPO_PUBLIC_API_ROUTE}/products?${query}`;
    try {
  ...
  const getNewArrivals = async () => {
    ...
    try {
      const res = await apiReqHandler({
        endPoint: `${process.env.EXPO_PUBLIC_API_ROUTE}/products?newArrival=true`,
        method: "GET",
      });
  ...
  const getProductsByImage = async (file: IProductImage) => {
    ...
    try {
      const res = await apiReqHandler({
        endPoint: `${process.env.EXPO_PUBLIC_API_ROUTE}/products/search-by-image`,
        method: "POST",
  ...
  const createReview = async (payload: IReview) => {
    ...
    try {
      const res = await apiReqHandler({
        endPoint: `${process.env.EXPO_PUBLIC_API_ROUTE}/review`,
        method: "POST",
  ...
  const getReviews = async (productId: string) => {
    setLoading(true);
    let url = `${process.env.EXPO_PUBLIC_API_ROUTE}/review/${productId}`;
    try {
      const res = await apiReqHandler({
        endPoint: url,
        method: "GET",
  ...
  const updateReview = async (payload: IReview, productId: string) => {
    ...
    try {
      const res = await apiReqHandler({
        endPoint: `${process.env.EXPO_PUBLIC_API_ROUTE}/review/${productId}`,
        method: "PUT",
        payload: JSON.stringify(payload),
      });
      setLoading(false);
